...
public class PdfGenerator {
    ...
    public void generatePDF(List<Employee> list) {
        ...
        try {
            ...
            for (Employee emp : list) {
                ...
                if (emp.getEmpName() != null) {
                    PdfPCell cell = new PdfPCell(new Phrase("Emp Name"));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setBackgroundColor(BaseColor.CYAN);
                    table.addCell(cell);
                }
                if (emp.getEmpDepartment() != null) {
                    PdfPCell cell = new PdfPCell(new Phrase("Emp Department"));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setBackgroundColor(BaseColor.CYAN);
                    table.addCell(cell);
                }
                if (emp.getEmpSalary() > 0) {
                    PdfPCell cell = new PdfPCell(new Phrase("Emp Salary"));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setBackgroundColor(BaseColor.CYAN);
                    table.addCell(cell);
                }
